 ...
 * @param {object} options - Policy options eg. imageHosts, styleHosts, scriptHosts, frameHosts, connectHosts
 */
const contentSecurityPolicy = (server, options = {}) => {
  ...
  server.use(helmet.contentSecurityPolicy({
    directives: {
      ...
      styleSrc: ["'self'", "'unsafe-inline'"].concat(formatHosts(options.styleHosts)),
      ...
      imgSrc: ["'self'", 'data:'].concat(formatHosts(options.imageHosts)),
      ...
      scriptSrc: ["'self'", "'unsafe-inline'", 'https://js.stripe.com', 'https://*.paypal.com', 'https://*.paypalobjects.com', "'unsafe-eval'"].concat(formatHosts(options.scriptHosts)),
      
      // Allow <frame> and <iframe>'s from third party sources.
      frameSrc: ['https://js.stripe.com', 'https://*.paypal.com'].concat(formatHosts(options.frameHosts)),
      
      // Disable loading using script interfaces
      // i.e. <a> pings, Fetch, XHR, WebSocket and EventSource
      connectSrc: ["'self'", 'https://*.algolia.net', 'https://*.algolianet.com', 'https://*.paypal.com'].concat(formatHosts(options.connectHosts)),
